---
slug: how-to-add-feature-flags-with-firebase-remote-config-in-react
title: How to add Feature Flags with Firebase Remote Config in React
date: '2020-10-02'
author: Fabian Lee
...
We need to install 2 packages for this demo. I'll explain what semver is for later.
...
// You can ignore this if you're manually deleting cache from the browser
...
We get the `remoteConfig` instance and adjust the settings of it. The reason we include the settings is because all the fetched remote configs will be cached in your browser's IndexDB. 

We can adjust the `minimumFetchIntervalMillis` which defines the maximum age of an entry in the config cache before it is considered stale. It's cached for 12 hours by default, we can change it to a shorter period in development. And obviously there's nothing stopping you from deleting the cache in IndexDB manually.
...
We would be taking a `defaults` prop to set the default values of your feature flags. We save them in a  state and pass it to our Provider below, so we can trigger a re-render when we set new flags.
...
In the `useEffect` hook, we assign our `defaults` prop as the default config. Then we fetch and activate all the remote flags. We would loop through the remote flags and set our `flags` state.
...
We would retrieve our app version from our environment variables. Simply add `REACT_APP_VERSION=$npm_package_version` to your `.env` file in this example which would retrieve the version number from your `package.json`.

We have 1 flag which is `['someAwesomeFeatureEnabled', '^1.0.0']`.
